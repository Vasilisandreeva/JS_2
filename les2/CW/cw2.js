// // - Користувач вводить або має два числа. Потрібно знайти та вивести максимальне число з тих двох. Також потрібно врахувати коли введені рівні числа.
// let num1 = +prompt('введіть число');
// let num2 = +prompt('введіть друге число');
// function max(num1, num2) {
//     return Math.max(num1,num2);
//     console.log(Math.max)
// }


// // - У першому під'їзді квартири з 1 до 20. У другому з 21 по 48. У третьому з 49 по 90. Користувач вводить номер квартири просто в змінні або через prompt('') .
// //     Програма повинна зазначити, в якому під'їзді знаходиться дана квартира.
// let entranse = +prompt('введіть номер квартири');
// if (entranse <= 20){
//     console.log(1);
// } else if(entranse <= 48){
//     console.log(2);
// } else if(entranse <= 90) {
//     console.log(3);
// } else {
//     console.log('шукайте інший будинок')
// }

// // Ми маємо змінну number в яку користувач задає числове значення,  якщо змінна рівна 10 вивести повідомлення ВІРНО, якщо змінна не рівна 10 - тоді НЕВІРНО
// let number = +prompt('введіть номер квартири');
// if (number === 10){
//     console.log('вірно');
// } else {
//     console.log('невірно');
// }

// // - Ми маємо змінну x в яку користувач задає  значення,  якщо в змінну записане число, вивести 1, якщо в змінну записали стрінгу вивести 2, якщо булеве значення - 3, якщо об'єкт або масив - 3.
// // якщо в змінну записали щось інше, спрацьовує else
// let x = prompt('введіть значення');
// if(typeof x === number){
//     console.log(1);
// } else if(typeof x === string){
//     console.log(2);
// } else if(typeof x === boolean){
//     console.log(3);
// } else {
//     console.log('що це?');
// }


// // - Задача-гра "чи ми йдемо сьогодні в OKTEN на навчання?".
// //     Змінна, яка характеризує температуру. Якщо температура від +10 до +22 вивести повідомлення що ми йдемо ВЧИТИСЯ .
// //     Якщо інша темпаретура, тоді виводимо повідомлення що сидимо вдома і вчимося ОНЛАЙН
// let tempreche = +prompt('яка температура?');
// if (tempreche === 10 ){
//     console.log('йдемо вчитись!');
// } else {
//     console.log('сидим вдома');
// }                                          //як поставити оцю вот градацію від... до?




// - За допомогою switch case - реалізувати гру - вікторину, де ми вводимо число від 1 до 5. І взалежності від введеного
//     числа нам дається приз. (Авто, Мото, Телефон, і т.д ) і якщо число не підходить тоді  вивести Повідомлення - що число не вірне ... .
// let game = prompt('введіть число');
// switch (game){
//     case '1':
//         console.log('авто');
//         break;
//     case '2':
//         console.log('велік');
//         break;
//     case '3':
//         console.log('мото');
//         break;
//     case '4':
//         console.log('телефон');
//         break;
//     case '5':
//         console.log('ноут');
//         break;
//     default:
//         alert('число не вірне');
// }
